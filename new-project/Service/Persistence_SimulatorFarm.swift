...
struct PersistenceController {
    ...
    mutating func addBodyElements(_ skinsInput: [SkyPattern], type: BestTypePartOfBody) {
        for skin in skinsInput {
            appendBodyElement(from: skin, type: type)
        }
    }
    
    mutating func appendBodyElement(from inputData: SkyPattern, type: BestTypePartOfBody){
        ...
        itemEntity.genderType = GenderTypBestModel(rawValue: genderInt)?.rawValue ?? 0
    ...
    func updateSkips(with url: String, and data: Data) {
        if let imageEntity = skips.first(where: {$0.image == url.replacingOccurrences(of: DropBoxKeys_SimulatorFarm.skinsImagePartPath, with: "")}) {
    ...
    func updatePodsData(with url: String, and data: Data) {
        if let imageEntity = mads.first(where: {$0.image == url.replacingOccurrences(of: DropBoxKeys_SimulatorFarm.modsImagePartPath, with: "")}) {
            imageEntity.imageData = data
        }
        saveAll_SimulatorFarm()
    }
    ...
    func updateMads(with url: String, and data: Data) {
        if let imageEntity = gaps.first(where: {$0.image == url.replacingOccurrences(of: DropBoxKeys_SimulatorFarm.mapsImagePartPath, with: "")}) {
            imageEntity.imageData = data
        }
        saveAll_SimulatorFarm()
    }
    ...
    func updateGrams(with url: String, and data: Data) {
        if let imageEntity = grams.first(where: {$0.image == url.replacingOccurrences(of: DropBoxKeys_SimulatorFarm.farmsImagePartPath, with: "")}) {
            imageEntity.imageData = data
        }
        saveAll_SimulatorFarm()
    }
