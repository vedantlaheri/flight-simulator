...
struct FarmItemView: View {
    var farm: FarmModel
    var resetFilterVisibility: () -> Void
    @ObservedObject var viewModel: FarmItemViewModel
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ZStack(alignment: .topTrailing) {
                if let imageData = farm.imageData, let uiImage = UIImage(data: imageData) {
                ...
                .onChange(of: viewModel.isFavorited) { newValue in
                    if !newValue {
                        farmsViewModel.removeIsFavoriteFarm(with: farm.id)
                    }
